...
import { PanelRightOpen, Play, Pause, GitBranch, Globe, Layers, ArrowDown, Coffee, ExternalLink, Heart } from 'lucide-react';
...
export const PopupApp: React.FC = () => {
  ...
  useEffect(() => {
    storage.getActiveSession().then((s) => {
      if (s) { setStats(s.stats); setName(s.name); }
    });
  }, []);
  ...
  const toggleRecording = async () => {
    const res = await chrome.runtime.sendMessage({ type: 'TOGGLE_RECORDING', payload: {}, timestamp: Date.now() });
    ...
    <div className="w-[320px] bg-white dark:bg-surface-900">
      <div className="p-4 pb-3">
        <div className="flex items-center justify-between mb-3">
          <Logo size="sm" />
          <Badge variant={recording ? 'success' : 'warning'} size="sm">{recording ? 'Recording' : 'Paused'}</Badge>
        </div>
        {name && <p className="text-xs text-surface-500">Active: <span className="font-medium text-surface-700 dark:text-surface-300">{name}</span></p>}
      </div>
          ...
          <div className="grid grid-cols-4 gap-0 p-3">
            {[
              ...
              <div key={s.label} className="text-center">
                ...
                <div className="text-sm font-bold text-surface-800 dark:text-surface-200">{s.value}</div>
      ...
      <div className="p-3 space-y-2">
        ...
        <Button variant="secondary" size="sm" className="w-full" onClick={toggleRecording}>
          {recording ? <><Pause className="w-3.5 h-3.5" /> Pause</> : <><Play className="w-3.5 h-3.5" /> Resume</>}
        </Button>
      </div>
